...
#define STREAM_COUNT 32  // Number of CUDA streams
...
int main(int argc, char *argv[]) {
    ...
    int verticesPerStream = (vertices + STREAM_COUNT - 1) / STREAM_COUNT;

    for (int i = 0; i < STREAM_COUNT; i++) {
        int edgeOffset = i * edgesPerStream;
        int edgeCount = (i == STREAM_COUNT - 1) ? (edges - edgeOffset) : edgesPerStream;

        int vertexOffset = i * verticesPerStream;
        int vertexCount = (i == STREAM_COUNT - 1) ? (vertices - vertexOffset) : verticesPerStream;

        cudaMemcpyAsync(deviceSrc + edgeOffset, src + edgeOffset, edgeCount * sizeof(int), cudaMemcpyHostToDevice, streams[i]);
        cudaMemcpyAsync(deviceDst + edgeOffset, dst + edgeOffset, edgeCount * sizeof(int), cudaMemcpyHostToDevice, streams[i]);
        cudaMemcpyAsync(deviceLevel + vertexOffset, level + vertexOffset, vertexCount * sizeof(int), cudaMemcpyHostToDevice, streams[i]);
    }
    cudaDeviceSynchronize();
    timer.stop();
    ...
    while (vertexVisited) {
        ...
        for (int i = 0; i < STREAM_COUNT; i++) {
            ...
            edge_centric_bfs<<<blocksPerGrid, threadsPerBlock, 0, streams[i]>>>(deviceSrc + edgeOffset, deviceDst + edgeOffset, deviceLevel, currentLevel, edgeCount, deviceVertexVisited);
        }
    ...
    for (int i = 0; i < STREAM_COUNT; i++) {
        int vertexOffset = i * verticesPerStream;
        int vertexCount = (i == STREAM_COUNT - 1) ? (vertices - vertexOffset) : verticesPerStream;
        cudaMemcpyAsync(level + vertexOffset, deviceLevel + vertexOffset, vertexCount * sizeof(int), cudaMemcpyDeviceToHost, streams[i]);
    }
    for (int i = 0; i < STREAM_COUNT; i++) {
        cudaStreamSynchronize(streams[i]);
    }

...
__global__ void edge_centric_bfs(int *src, int *dst, int *level, int currentLevel, int vertices, int edges, int *vertexVisited)
{
    int edge = blockIdx.x * blockDim.x + threadIdx.x; // each thread is assigned an edge (element in the array)

    if (edge < edges)
    {
        int sourceVertex = src[edge];
        int destinationVertex = dst[edge];
...
int main(char argc, char *argv[])
{
    ...
    while (vertexVisited)
    {
        ...
        edge_centric_bfs<<<threadsPerBlock, blocksPerGrid>>>(deviceSrc, deviceDst, deviceLevel, currentLevel, vertices, edges, deviceVertexVisited);
